...
def readPressure(path):
	...
	return times_pressure, pressure
def alignSignals(*t_x, timeRange = None):
	'''returns the timed signals in the largest range in which all of them are defined, with a sampling that is the smallest amongst them
	returned values: t, *x (t is shared between the output signals)'''
	ts = [t for t, x in t_x]
	xs = [x for t, x in t_x]
	if timeRange is not None:
		minTime, maxTime = timeRange
	else:
		minTime = np.max([np.min(t) for t in ts])
		maxTime = np.min([np.max(t) for t in ts])
	dt = np.max([(np.max(t) - np.min(t)) / len(t) for t in ts])
	t = np.linspace(minTime, maxTime, int((maxTime - minTime) / dt))
	#I should filter before sampling...
	xs = [np.interp(t, ts[i], xs[i]) for i in range(len(xs))]
	return t, xs
if __name__ == "__main__":
	...
	times_pressure, pressure = readPressure(pressure_path)
