...
def getSpectrumAnalysis_signalHound_RIN(file_csv, g1, g2, ge, photodiodeResponsivity, laserPower = None, isDataIndB = True, outputImpedance_Ohm = 50, inputInpedance = 50, ):
	'''if laserPower is not specified, either the laser power or the tia setpoint must be specified in the name of the file'''
	
	if laserPower is None:
		name = os.path.basename(file_csv)
		
		extractedNumber, stringFound = extract_setpoint_number(name.replace(".csv",""))
		if stringFound == "setpoint":
			currentSetpoint = extractedNumber#          V
			currentPower = currentSetpoint * ge / g2 / g1 / photodiodeResponsivity# maxPower * currentSetpoint / maxSetPoint#          W
			
		elif stringFound == "laserPower":
			currentPower = extractedNumber * 1e-3#laser power given in mW#          W
			currentSetpoint = currentPower * g1 * g2 / ge
	currentVoltage = currentSetpoint * ge
	baseVoltage_dBm = V_sqrtHz_to_dBm(currentVoltage) - 10*np.log10(2)#          dBm(V^2*Ohm/W)
	
	x, y = getSpectrumAnalysis_signalHound(file_csv)#          V / sqrt(Hz)
	
	y = V_sqrtHz_to_dBm(y)#          dBm(V^2*Ohm/W) / Hz
